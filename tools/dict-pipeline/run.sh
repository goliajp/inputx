...
ENGINES=(pinyin wubi)
# Sources to harvest + diff. v1.9.0 WU-π.b: added zh-opensubtitles to
# complement wiki — subtitle dialog gives IME-relevant casual register,
# wiki gives formal-prose breadth. Each source runs through its own
# harvester (manifest.yaml binds source name → script), produces an
# independent TSV, and gets diffed against each engine separately.
SOURCES=(zh-wikipedia zh-opensubtitles)

# 1. Harvest each source
for SOURCE in "${SOURCES[@]}"; do
    ...
    case "$SOURCE" in
        zh-wikipedia)
            ...
            ;;
        zh-opensubtitles)
            # OPUS v2018 is a frozen release; date param doesn't apply.
            # The harvester stamps the run's date for provenance only.
            python3 tools/corpus-harvest/harvest_zh_opensubtitles.py $DRY_RUN_FLAG 2>&1 \
                | grep -E "^=== stats|lines_parsed|unique_words|output_path|wrote" | head -8
            ;;
        *)
            echo "[dict-pipeline] ERROR: no harvester wired for $SOURCE" >&2
            exit 1
            ;;
    esac
done

# 2. Resolve harvest output paths + diff against each engine
DATE_EFFECTIVE_WIKI="${DATE:-20260501}"  # mirrors manifest sample_date default
DATE_EFFECTIVE_SUBS="$(date -u +%Y%m%d)"  # subtitles harvester uses today's UTC date

for SOURCE in "${SOURCES[@]}"; do
    case "$SOURCE" in
        zh-wikipedia)     DATE_PART="$DATE_EFFECTIVE_WIKI" ;;
        zh-opensubtitles) DATE_PART="$DATE_EFFECTIVE_SUBS" ;;
    esac
    HARVEST_TSV="tools/corpus-harvest/output/${SOURCE}/${DATE_PART}${SUFFIX}.tsv"
...
# 3. Summary — one row per (source × engine) pair
echo ""
