...
/// Word-trigram FST: keys = `<a>\0<b>\0<c>` (UTF-8), values = raw
/// co-occurrence count. Generated by `pinyin-build-trigrams-fst`. Used
/// by `predict_next_words_context` for sentence-level coherent
/// next-word prediction — bigram-only prediction tends to "接龙到死"
/// (e.g. 今天→的→是→在→年, each link locally optimal but the chain
/// isn't a sentence). Trigrams condition on the last TWO committed
/// words for sharper context.
#[cfg(not(feature = "bootstrap_only"))]
...
pub struct PinyinDict {
    ...
    /// Optional word-trigram FST. Same bootstrap_only caveat.
...
impl PinyinDict {
    ...
    pub fn embedded() -> Self {
        Self {
            ...
            trigrams: if TRIGRAMS_BYTES.is_empty() {
                None
            } else {
                Some(Map::new(TRIGRAMS_BYTES).expect("invalid embedded trigrams FST"))
            },
    ...
    /// Context-aware next-word prediction. Like `predict_next_words` but
    /// also takes the word committed BEFORE `prev` so the model can use
    /// trigram (prev_prev, prev, *) context for sharper picks.
    ///
    /// Algorithm: trigram-first with bigram backoff (standard interpolated
    /// LM pattern, just discrete — no smoothing math needed for our use
    /// case).
    ///
    ///   1. If `prev_prev` is `Some` AND trigrams FST is loaded: scan
    ///      `[prev_prev\0prev\0, prev_prev\0prev\1)` for trigram hits.
    ///      Return up to `limit` candidates sorted by count desc.
    ///   2. If trigram path returns fewer than `limit` results (or no
    ///      `prev_prev` available), fall back to bigram (`predict_next_
    ///      words(prev, limit)`) and merge — trigram candidates are
    ///      always preferred when both sources return the same word.
    ///   3. Returns empty Vec when both paths are empty (very rare
    ///      context, English / kana, etc.).
    ///
    /// Why interpolated rather than pure trigram: trigrams are SPARSE —
    /// even with 1M trigrams indexed, many real (prev_prev, prev) pairs
    /// have no followers in the corpus. Pure-trigram with no backoff
    /// would silently die when context goes off-corpus. Backoff to
    /// bigram keeps predictions flowing while letting trigrams sharpen
    /// when available.
    pub fn predict_next_words_context(
        ...
        // Step 1: trigram lookup (when both context words are available
        // and the trigram FST is loaded).
        if let (Some(prev_prev), Some(trigrams)) = (prev_prev, self.trigrams.as_ref())
            && !prev_prev.is_empty()
        {
            ...
            hits.sort_by(|a, b| b.1.cmp(&a.1).then_with(|| a.0.cmp(&b.0)));
            for (word, count) in hits.into_iter().take(limit) {
        ...
        // Step 2: bigram backoff to fill remaining slots.
        if out.len() < limit {
            let need = limit - out.len();
            for (word, count) in self.predict_next_words(prev, need * 2) {
                if seen.insert(word.clone()) {
                    out.push((word, count));
                }
                if out.len() >= limit { break }
...
mod tests {
    ...
    #[cfg(not(feature = "bootstrap_only"))]
    #[test]
    fn predict_next_words_context_backoff_to_bigram() {
        let d = PinyinDict::embedded();
        // For an unusual prev_prev that has no trigram followers
        // through `prev=我们`, the backoff should still return
        // bigram-based predictions for (我们, *).
        let bare = d.predict_next_words("我们", 5);
        let backoff = d.predict_next_words_context(
            Some("锟斤拷"), "我们", 5);
        assert!(!backoff.is_empty(),
            "expected bigram backoff when trigram path is empty");
        // Backoff result should match bigram-only result (modulo
        // count/order details) — same data path.
        assert_eq!(backoff.len(), bare.len(),
            "backoff length should match pure bigram");
    }
